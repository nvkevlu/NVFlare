...
class TestCanonicalReviewArtifacts(unittest.TestCase):
    def test_committed_tree_is_coherent_and_reproducible(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual("480", receipt["derived_examples"]["gpu_instance_seconds"])
    ...
    def test_default_cli_is_compact_and_hides_inapplicable_mig(self):
        ...
        self.assertIn("job coverage: PARTIAL (1 accepted / 2 expected)", human)
        ...
        self.assertIn("0.1333", human)
        self.assertIn("136.5333", human)
        self.assertIn("REPORTED", human)
        self.assertNotIn("Site QUALITY PARTIAL", human)
        self.assertIn("SAVED RESULT MiB", human)
        self.assertIn("F3 REMOTE ACCEPTED KiB", human)
        ...
        partial_periods = (COMMITTED_ROOT / "cli" / "resources-partial-periods.txt").read_text()
        self.assertIn("accepted  PARTIAL", partial_periods)
        self.assertIn("Site QUALITY PARTIAL means that site's measurement evidence is incomplete", partial_periods)
        self.assertNotIn("JOB COVERAGE PARTIAL", partial_periods)
