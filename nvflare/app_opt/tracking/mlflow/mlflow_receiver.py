...
class MLflowReceiver(AnalyticsReceiver):
    def __init__(
        ...
        Args:
            tracking_uri (str): MLflow tracking server URI. When this is not specified, the metrics will be written to the local file system.
            ...
            artifact_location (str): Relative location of artifacts. Currently only text is supported at the moment.

            events (str): The event the receiver is listening to. By default, it listens to "fed.analytix_log_stats".

            buffer_flush_time (float): The time in seconds between deliveries of event data to the MLflow tracking server. The
    ...
    def mlflow_setup(self, art_full_path, experiment_name, experiment_tags, sites):
        """Set up a MlflowClient for each client site and create an experiment and run.
