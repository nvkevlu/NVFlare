...
def test_configure_f3_loads_chunk_and_window_sizes(tmp_path):
    ...
    try:
        ...
        assert configurator.get_tcp_verify_hostname(False) is True
        assert configurator.get_tcp_tensor_bulk_enabled(False) is True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tcp_verify_hostname: yes-please\n", "tcp_verify_hostname must be a boolean"),
        ("tcp_tensor_bulk_enabled: true\n", "requires tcp_verify_hostname"),
...
def test_native_tls_profile_enables_three_bulk_lanes_on_one_stcp_listener():
    ...
    assert config["tcp_verify_hostname"] is True
    assert config["tcp_tensor_bulk_enabled"] is True
    assert config["tcp_tensor_bulk_lanes"] == 3
    assert config["streaming_window_size"] == 64 * cellnet_bench.MB
